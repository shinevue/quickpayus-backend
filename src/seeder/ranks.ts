        ...
        for (let i = 1; i < users.length; i++) {
            ...
            for (let j = 0; j < 1; j++) {
                for (let k = 0; k < 1; k++) {
                    ...
                    reward.rankId = ranks[Math.ceil(Math.random() * 4)]._id; // Assuming rankId should be the _id of the rank
                    reward.status = faker.helpers.arrayElement(Object.values(STATUS));
                    reward.reason = reward.status === "REJECTED" ? faker.lorem.word() : null; // Generate a random reason
                    reward.isClaimed = reward.status === 'PENDING';
                    reward.amount = 1000 + Math.ceil(Math.random() * 1000);
                    reward.createdAt = date.toDate(); // Convert moment object to JavaScript Date
                    await reward.save();
                    console.log(`${i}----------${j}------------${k}`);
                }
                date.add(1, "days");
